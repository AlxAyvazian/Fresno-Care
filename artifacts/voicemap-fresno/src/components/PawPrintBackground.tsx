...
const PAWS: Paw[] = Array.from({ length: 34 }, (_, index) => {
  ...
  return {
    ...
    style: {
      ...
      "--rotation-mid": `${rotation + 2}deg`,
      "--rotation-end": `${rotation + 5}deg`,
      "--drift-start-x": `${Math.round(driftX * -0.35)}px`,
      "--drift-start-y": `${Math.round(driftY * -0.35)}px`,
      "--drift-mid-x": `${Math.round(driftX * 0.45)}px`,
      "--drift-mid-y": `${Math.round(driftY * 0.45)}px`,
      "--drift-x": `${driftX}px`,
      "--drift-y": `${driftY}px`,
    },
  };
